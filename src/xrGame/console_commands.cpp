...
public:
	...
	virtual void	Execute				(LPCSTR args) {
		...
		for ( ; I != E; ++I) {
			auto obj = smart_cast<CSE_ALifeCreatureAbstract *>(I->second);
			if (obj) {
...
struct CCC_StartTimeSingle: IConsole_Command {
	...
	virtual void	Execute	(LPCSTR args)
	{
		...
		year				= _max(static_cast<s32>(year), 1);
		month				= _max(static_cast<s32>(month),1);
		day					= _max(static_cast<s32>(day),1);
...
struct CCC_TimeFactorSingle: CCC_Float {
...
public:
	...
	virtual void Execute( LPCSTR args )
	{
		...
		auto ui_game_sp = smart_cast<CUIGameSP*>( CurrentGameUI() );
		if ( !ui_game_sp )
		{
			return;
		}
