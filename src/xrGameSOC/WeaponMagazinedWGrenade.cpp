...
void CWeaponMagazinedWGrenade::OnShot		()
{
	if(m_bGrenadeMode)
	{
		...
		//партиклы огн€ вылета гранаты из подствольника
...
bool CWeaponMagazinedWGrenade::Attach(PIItem pIItem, bool b_send_event)
{
	   ...
	   !xr_strcmp(*m_sGrenadeLauncherName, pIItem->object().cNameSect()))
	{
		...
		CRocketLauncher::m_fLaunchSpeed = pGrenadeLauncher->GetGrenadeVel();
 		...
 		//уничтожить подствольник из инвентар€
		if(b_send_event)
		{
//.			pIItem->Drop();
			if (OnServer()) 
...
void CWeaponMagazinedWGrenade::PlayAnimShoot()
{
	...
	{
		//анимаци€ стрельбы из подствольника
...
void CWeaponMagazinedWGrenade::net_Import(NET_Packet& P) //Ётот и все подобные методы вообще не вызываютс€ в в синглплеере, походу.
{
	u8 _data = P.r_u8();
