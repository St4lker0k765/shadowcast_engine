...
void CMapLocation::UpdateSpot(CUICustomMap* map, CMapSpot* sp )
{
	if( map->MapName() == GetLevelName() )
	{
		...
		if ( IsGameTypeSingle() )
		{
			...
			if ( ml_task && ml_task->GetTaskType() == eTaskTypeStoryline )
			{
				CGameTask* active_task = Level().GameTaskManager().ActiveTask( eTaskTypeStoryline );
				if ( ml_task != active_task )
				{
					return;
				}
			}

			if ( ml_task && ml_task->GetTaskType() == eTaskTypeAdditional )
			{
				CGameTask* active_task2 = Level().GameTaskManager().ActiveTask( eTaskTypeAdditional );
				bool border_show = ( ml_task == active_task2 );
				if ( m_minimap_spot )	{	m_minimap_spot->show_static_border( border_show );	}
				if ( m_level_spot )
				{
					m_level_spot->show_static_border( border_show );
					//if ( !ml_task->m_read ){m_level_spot->mark_focused();}
				}
				if ( m_complex_spot )
				{
