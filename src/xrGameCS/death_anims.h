...
public:	
	type_motion			( ){}
virtual				~type_motion		( );
		type_motion	*setup				( IKinematicsAnimated* k, const CInifile* ini, LPCSTR section, LPCSTR type );
...
public:
					death_anims	( );
					~death_anims( );
	void			setup		( IKinematicsAnimated* k, LPCSTR section, const CInifile* ini );
	void			clear		( );
