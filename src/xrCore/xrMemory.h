...
IC void* operator new (size_t size) noexcept(false) { return Memory.mem_alloc(size ? size : 1, "C++ NEW"); }
...
#else // DEBUG_MEMORY_NAME
# if !(defined(__BORLANDC__) || defined(NO_XRNEW))
IC void* operator new (size_t size) noexcept(false) { return Memory.mem_alloc(size ? size : 1); }
IC void operator delete (void* p) noexcept { xr_free(p); }
IC void* operator new[](size_t size) noexcept(false) { return Memory.mem_alloc(size ? size : 1); }
IC void operator delete[](void* p) noexcept { xr_free(p); }
# endif
